...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.GET_BETWEEN, query = "SELECT m FROM Meal m  WHERE m.user=:user and m.dateTime between :startDate and :endDate ORDER BY m.dateTime desc")
})
@Entity
@Table(name = "meals", uniqueConstraints = {@UniqueConstraint(columnNames = {"id","user_id"}, name = "meals_unique_user_datetime_idx")})
